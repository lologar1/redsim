...
#define HOTBAR_SLOT_VERTICES (f32 []) { \
...
#define HOTBAR_SLOT_INDICES (u32 []) {0 + i*4, 1 + i*4, 2 + i*4, 0 + i*4, 2 + i*4, 3 + i*4}
		memcpy(hotbarv + i * 4 * 5, HOTBAR_SLOT_VERTICES, 4 * 5 * sizeof(f32));
		memcpy(hotbari + i * 6, HOTBAR_SLOT_INDICES, 6 * sizeof(u32));
...
#define SELECTION_INDICES (u32 []) {0, 1, 2, 0, 2, 3}
	gu_meshSet(mSlotSelection,
			SELECTION_VERTICES, sizeof(SELECTION_VERTICES)/sizeof(f32),
			SELECTION_INDICES, sizeof(SELECTION_INDICES)/sizeof(u32));
#undef SELECTION_VERTICES
#undef SELECTION_INDICES
}
...
#define INVENTORY_SLOT_VERTICES (f32 []) { \
...
#define INVENTORY_SLOT_INDICES (u32 []) {0 + i*4, 1 + i*4, 2 + i*4, 0 + i*4, 2 + i*4, 3 + i*4}
			memcpy(invv + i * 20, INVENTORY_SLOT_VERTICES, 20 * sizeof(f32));
			memcpy(invi + i * 6, INVENTORY_SLOT_INDICES, 6 * sizeof(u32));
...
#undef INVENTORY_SLOT_VERTICES
	}

	for (x = 0; x < RSM_INVENTORY_SUBMENUS; x++, i++) {
#define INVENTORY_SUBMENU_VERTICES (f32 []) { \
	...
	INV_BASEX+ICONSZ*(x+1), INV_SUBMENUY, mInventory, 1.0f, gu_guiAtlasAdjust(0.0f, TICONID+x) }
		memcpy(invv + i * 20, INVENTORY_SUBMENU_VERTICES, 20 * sizeof(f32));
		memcpy(invi + i * 6, INVENTORY_SLOT_INDICES, 6 * sizeof(u32)); /* Submenus are squares too */
	}
#undef INVENTORY_SLOT_INDICES /* Reused as index order is the same for submenus */
#undef INVENTORY_SUBMENU_INDICES
...
#define SPRITE_VERTICES (f32 []) { \
	...
	memcpy(spritevbuf_ + nspritev_, SPRITE_VERTICES, 20 * sizeof(f32));
#undef SPRITES_PER_LINE
#undef SPRITE_STRIDE
#undef SPRITE_VERTICES
